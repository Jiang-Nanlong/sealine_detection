...
def load_split_indices() -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Load train/val/test indices for SMD"""
...
def build_smd_datasets(stage: str):
    """Build SMD datasets for training"""
    ...
    full_ds_train = ExternalDataset(
        SMD_CSV, SMD_IMG_DIR, img_size=IMG_SIZE, 
    ...
    print(f"[SMD Split] Train(Aug=True)={len(train_ds)} | Val(Aug=False)={len(val_ds)} | Test={len(test_idx)}")
...
def load_checkpoint_smart(model, current_stage: str, device: str):
    ...
    for prev_stage, kind in priority_map[current_stage]:
        ...
        if kind == "best_joint":
            fname = os.path.join(WEIGHTS_DIR, f"smd_rghnet_best_{prev}.pth")
        elif kind == "best_seg":
            fname = os.path.join(WEIGHTS_DIR, f"smd_rghnet_best_seg_{prev}.pth")
        elif kind == "last":
            fname = os.path.join(WEIGHTS_DIR, f"smd_rghnet_last_{prev}.pth")
        else:
            continue
    ...
    for batch in loader:
        if mode == "seg":
            ...
            with amp.autocast(device_type=DEVICE_TYPE, enabled=(DEVICE_TYPE == "cuda")):
                _, seg, _, _, _ = model(img, None, enable_restoration=True, enable_segmentation=True)
            ...
            mask = mask.to(DEVICE, non_blocking=True)
            with amp.autocast(device_type=DEVICE_TYPE, enabled=(DEVICE_TYPE == "cuda")):
                restored, seg, _, _, _ = model(img, target, enable_restoration=True, enable_segmentation=True)
            ...
            target = target.to(DEVICE, non_blocking=True)
            with amp.autocast(device_type=DEVICE_TYPE, enabled=(DEVICE_TYPE == "cuda")):
                restored, _, _, _, _ = model(img, target, enable_restoration=True, enable_segmentation=False)
                loss_r = crit_rest(restored, target)
            mse = F.mse_loss(restored.detach().float(), target.detach().float())
            sums["psnr"] += psnr_from_mse(mse)
            sums["rest"] += float(loss_r.item())
...
def main():
    ...
    print(f"=== SMD UNet Training: Stage {STAGE} ===")
    print(f"DEVICE={DEVICE}, BS={BATCH_SIZE}")
    
    # Build datasets
    train_ds, val_ds, eval_mode = build_smd_datasets(STAGE)
    ...
    best_joint_name = os.path.join(WEIGHTS_DIR, f"smd_rghnet_best_{STAGE.lower()}.pth")
    best_seg_name = os.path.join(WEIGHTS_DIR, f"smd_rghnet_best_seg_{STAGE.lower()}.pth")
    last_name = os.path.join(WEIGHTS_DIR, f"smd_rghnet_last_{STAGE.lower()}.pth")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if STAGE == "C2":
            curr_seg_w = 1.0  # C2阶段始终保持seg_w=1.0
