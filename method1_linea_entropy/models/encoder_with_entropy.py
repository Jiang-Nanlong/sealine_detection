...
from method1_linea_entropy.models.entropy_branch import (
    EntropyBranch,
    MultiScaleEntropyBranch,
    MultiScaleEntropyPriorBranch,
    SpatialAdaptiveInjection,
    EntropyGuidedAttentionBias,
)
...
# ============================================================
# 增强版 Encoder: MSLEP + SAI (用于 LINEA_ENTROPY_B 增强)
# ============================================================
class HybridEncoderWithEntropyEnhanced(HybridEncoderAsymConv):
    """
    增强版 encoder, 在 HybridEncoderWithEntropy 基础上新增:
      1. MSLEP (Multi-Scale Local Entropy Prior) — 多尺度熵先验提取
      2. SAI (Spatial Adaptive Injection) — 空间自适应注入

    entropy_map 输入约定:
      - entropy_in_channels=1 时, 接受 [B, 1, H, W] (向后兼容, 等同原始 EntropyBranch)
      - entropy_in_channels=3 时, 接受 [B, 3, H, W] (多尺度熵图, 如 5x5/11x11/21x21 窗口)

    注入方式:
      - use_sai=False: proj_feats[0] += alpha * entropy_feat (兼容原始方式)
      - use_sai=True : proj_feats[0] = SAI(proj_feats[0], entropy_feat) (空间自适应)
    """

    def __init__(self, *args, entropy_in_channels=1, use_sai=True, **kwargs):
        super().__init__(*args, **kwargs)

        self.entropy_in_channels = entropy_in_channels
        self.use_sai = use_sai

        # 熵特征提取
        if entropy_in_channels == 1:
            self.entropy_branch = EntropyBranch()
        else:
            self.entropy_branch = MultiScaleEntropyPriorBranch(in_channels=entropy_in_channels)

        # 注入方式
        if use_sai:
            self.sai = SpatialAdaptiveInjection(channels=256)
        else:
            self.alpha = nn.Parameter(torch.tensor(0.0))
    ...
    def forward(self, feats, entropy_map=None):
        ...
        Args:
            feats: list of Tensor — backbone 多尺度输出
            entropy_map: [B, C_ent, H, W] — 局部熵图 (1 或 3 通道)
        Returns:
            outs: list of Tensor — 增强后的多尺度特征
            entropy_feat: Tensor or None — 提取的熵特征 (供后续 EGAB 使用)
        """
        assert len(feats) == len(self.in_channels)
        proj_feats = [self.input_proj[i](feat) for i, feat in enumerate(feats)]
        ...
        if entropy_map is not None:
            ...
            if self.use_sai:
                proj_feats[0], _ = self.sai(proj_feats[0], entropy_feat)
            else:
                proj_feats[0] = proj_feats[0] + self.alpha * entropy_feat
...
def build_hybrid_encoder_with_entropy_enhanced(args):
    """构建 HybridEncoderWithEntropyEnhanced."""
    return HybridEncoderWithEntropyEnhanced(
        in_channels=args.in_channels_encoder,
        feat_strides=args.feat_strides,
        n_levels=args.num_feature_levels,
        hidden_dim=args.hidden_dim,
        ...
        temperatureW=args.pe_temperatureW,
        eval_spatial_size=args.eval_spatial_size,
        entropy_in_channels=getattr(args, 'entropy_in_channels', 1),
        use_sai=getattr(args, 'use_sai', True),
    )
