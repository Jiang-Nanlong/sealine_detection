"""
ablation_mslep_only_musid.py — 消融实验：仅 MSLEP（多尺度局部熵先验）

用法：
  python LINEA/main.py -c method1_linea_entropy/configs/ablation_mslep_only_musid.py

模型：LINEA_ENTROPY_B_ENHANCED（关闭 SAI / EGAB / HASH）
  - MSLEP: 多尺度局部熵先验提取 (3 通道: 5×5, 11×11, 21×21 窗口)  ✓
  - SAI:   关闭，退化为 alpha 加法注入                              ✗
  - EGAB:  关闭                                                     ✗
  - HASH:  关闭                                                     ✗
数据：MU-SID，letterbox 到 640×640，entropy_mode='entropy_ms'

消融表第 3 行：Baseline → 单尺度熵 → **MSLEP**
"""
...
# 输出目录
# ============================================================
output_dir = 'output/ablation_mslep_only_musid_e130'
