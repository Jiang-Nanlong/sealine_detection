"""
train_unet.py (Unified 16:9 Version)
...
本版本关键改动：
1) IMG_SIZE 改为 (576, 1024) —— 适配所有 1920x1080，避免 padding 伪影
2) dataset 传入 p_clean —— 让模型学会 clean 时尽量 identity
3) 采用方案B：保留 DCE，但 restoration loss 用 target（而不是 target_dce）
"""
...
# ✅ 统一 16:9 输入（无 padding）
IMG_SIZE = (576, 1024)   # (H, W)
BATCH_SIZE = 4

# NEW: clean 直通比例（推荐 0.3~0.5）
P_CLEAN = 0.40
...
def build_musid_datasets(stage: str):
    ...
    full_ds_train = HorizonImageDataset(
        CSV_PATH, IMG_DIR,
        img_size=IMG_SIZE,
        mode=mode,
        augment=True,
    ...
    full_ds_val = HorizonImageDataset(
        CSV_PATH, IMG_DIR,
        img_size=IMG_SIZE,
        mode=mode,
        augment=False,
...
def evaluate(model, loader, mode: str, crit_rest, crit_seg, seg_w: float = 0.5) -> EvalResult:
    ...
    for batch in loader:
        if mode == "seg":
            ...
            with amp.autocast(device_type=DEVICE_TYPE, enabled=(DEVICE_TYPE == "cuda")):
                restored, seg, _target_dce = model(img, target, enable_restoration=True, enable_segmentation=True)
                # ✅ 方案B：restoration loss 对齐原始 target（不是 target_dce）
            ...
            with amp.autocast(device_type=DEVICE_TYPE, enabled=(DEVICE_TYPE == "cuda")):
                restored, _, _target_dce = model(img, target, enable_restoration=True, enable_segmentation=False)
                # ✅ 方案B：restoration loss 对齐原始 target
                loss_r = crit_rest(restored, target)
...
def main():
    ...
    print(f"LR={lr}, EPOCHS={epochs}, IMG_SIZE={IMG_SIZE}, BS={BATCH_SIZE}, P_CLEAN={P_CLEAN}")
        ...
        for batch in loop:
            ...
            if STAGE == "A":
                ...
                with amp.autocast(device_type=DEVICE_TYPE, enabled=(DEVICE_TYPE == "cuda")):
                    restored, _, _target_dce = model(img, target, enable_restoration=True, enable_segmentation=False)
                    # ✅ 方案B：对齐原始 target
                    loss = crit_rest(restored, target)
                ...
                with amp.autocast(device_type=DEVICE_TYPE, enabled=(DEVICE_TYPE == "cuda")):
                    restored, seg, _target_dce = model(img, target, enable_restoration=True, enable_segmentation=True)
                    loss_r = crit_rest(restored, target)  # ✅ 方案B
                    loss_s = crit_seg(seg, mask)
