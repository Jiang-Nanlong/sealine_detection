...
def evaluate(model, loader, mode: str, crit_rest, crit_seg, seg_w: float = 0.5) -> EvalResult:
    ...
    for batch in loader:
        if mode == "seg":
            ...
            with amp.autocast(device_type=DEVICE_TYPE, enabled=(DEVICE_TYPE == "cuda")):
                restored, seg, target_dce, _, _ = model(img, target, enable_restoration=True, enable_segmentation=True)
...
def main():
    ...
    for epoch in range(1, epochs + 1):
        ...
        if STAGE == "C2": curr_seg_w = 1.0  # C2阶段始终保持seg_w=1.0
        ...
        for batch in loop:
            ...
            if STAGE=="A":
                ...
                with amp.autocast(device_type=DEVICE_TYPE, enabled=(DEVICE_TYPE=="cuda")):
                    _, s, _, _, _ = model(img, None, True, True)
