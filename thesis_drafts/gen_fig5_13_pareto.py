...
methods = {
    'UNet+Radon+CNN':       {'fps': 0.136, 'acc': 36.69, 'color': '#E74C3C', 'marker': 's'},
    'LINEA-L':              {'fps': 2.3,   'acc': 40.0,  'color': '#8E44AD', 'marker': 'D'},
    'LINEA-N':              {'fps': 5.0,   'acc': 39.5,  'color': '#3498DB', 'marker': 'D'},
...
methods_unified = {
    'UNet+Radon+CNN':       {'fps': 0.136, 'score': 63.3, 'color': '#E74C3C', 'marker': 's', 'metric': 'VE=36.7px'},
    'LINEA-L':              {'fps': 2.3,   'score': 94.4, 'color': '#8E44AD', 'marker': 'D', 'metric': 'sAP@10=94.4'},
    'LINEA-N':              {'fps': 5.0,   'score': 94.7, 'color': '#3498DB', 'marker': 'D', 'metric': 'sAP@10=94.7'},
...
for name, d in methods_unified.items():
    ...
    elif 'LINEA-L' in name:
        offset_x, offset_y = 1.2, -2.5
    elif 'LINEA-N' in name:
        offset_x, offset_y = 1.2, 1.5
...
# 注释说明不同精度体系
ax.text(0.02, 0.02, 'Note: LINEA uses sAP@10 (higher=better); UNet uses 100-VE (higher=better)',
