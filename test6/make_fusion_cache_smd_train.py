# -*- coding: utf-8 -*-
"""
Build FusionCache for SMD training (Experiment 6: In-Domain Training).

This script generates FusionCache for SMD train/val/test sets.
Uses MU-SID pretrained UNet weights for feature extraction.

Inputs:
  - test4/SMD_GroundTruth.csv
  - test4/smd_frames/
  - test6/splits_smd/

Outputs:
  - test6/FusionCache_SMD/{train,val,test}/<idx>.npy

Usage:
  python test6/make_fusion_cache_smd_train.py
"""
...
# PyCharm 配置区
# ============================
# 使用手动筛选后的 SMD 数据 (manual_review 目录)
...
SAVE_ROOT = str(PROJECT_ROOT / "test6" / "FusionCache_SMD")

# ✅ 使用 SMD 本地训练的 UNet 权重 (In-Domain Training)
RGHNET_CKPT = str(PROJECT_ROOT / "test6" / "weights_smd" / "smd_rghnet_best_seg_c2.pth")
...
def main():
    ...
    print("=" * 60)
    print("Build FusionCache for SMD Training (Experiment 6)")
    print("=" * 60)

    # Check paths
    if not os.path.exists(CSV_PATH):
        print(f"[Error] CSV not found: {CSV_PATH}")
        print("  Please run test4/prepare_smd_testset.py first.")
        return 1
    ...
    if not os.path.isdir(SPLIT_DIR):
        ...
        print("  Please run test6/prepare_smd_trainset.py first.")
        return 1
