# -*- coding: utf-8 -*-
"""
Build FusionCache for Buoy dataset (Experiment 4: zero-shot generalization).

This script is a Buoy-adapted version of make_fusion_cache_smd.py.
It expects outputs from: test4/prepare_buoy_testset.py

Inputs (relative to project root):
  - test4/Buoy_GroundTruth.csv    (header: img_name,x1,y1,x2,y2,video)
  - test4/buoy_frames/            (extracted frames)
  - test4/splits_buoy/            (train/val empty, test all)

Outputs:
  - test4/FusionCache_Buoy/<train,val,test>/<idx>.npy

Notes:
  - Uses MU-SID trained weights (same as SMD evaluation).
  - Labels are computed in the UNet input space (1024x576).
...
SPLIT_DIR = str(PROJECT_ROOT / "test4" / "splits_buoy")
SAVE_ROOT = str(PROJECT_ROOT / "test4" / "FusionCache_Buoy")
...
def _read_image_buoy(img_dir: str, img_name: str):
    """Read image from buoy_frames directory."""
...
def calculate_radon_label(x1, y1, x2, y2, img_w, img_h, resize_h, resize_w):
    ...
    Label space follows existing code:
...
def build_cache_for_split(df: pd.DataFrame, indices, out_dir: str, seg_model, detector, theta_scan):
    ...
    for idx in tqdm(indices, ncols=80, desc=split_name):
        ...
        bgr = _read_image_buoy(IMG_DIR, img_name)
        if bgr is None:
...
def main():
    ...
    print("=" * 60)
    print("Build FusionCache for Buoy Dataset")
    print("=" * 60)

    # Check paths
    if not os.path.exists(CSV_PATH):
        print(f"[Error] CSV not found: {CSV_PATH}")
        print("  Please run prepare_buoy_testset.py first.")
        return
