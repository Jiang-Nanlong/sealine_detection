# -*- coding: utf-8 -*-
"""
visualize_buoy_predictions.py

Visualize horizon detection predictions on Buoy dataset for Experiment 4.
...
Useful for:
  ...
  3. Comparing video performance visually

Usage:
  python test4/visualize_buoy_predictions.py
"""
...
MODE = "worst"                  # Mode: "random" / "best" / "worst" / "per_video"
...
OUT_DIR = TEST4_DIR / "visualization_buoy"
...
def create_visualization(img_path, rho_pred, theta_pred, rho_gt, theta_gt, metrics, video):
    ...
    # Text - shorten video name for display
    video_short = video.replace("buoyGT_", "").replace("_", "-")
    cv2.putText(img, f"Video: {video_short}", (10, 25), FONT, FONT_SCALE, COLOR_TEXT, 1)
...
def main():
    ...
    print(f"[Info] Total samples: {len(eval_df)}")
    print(f"[Info] Videos: {eval_df['video'].nunique()}")
    ...
    if mode == "worst":
        ...
        # Select n_samples from each video
        selected_list = []
        videos = eval_df["video"].unique()
        n_per_video = max(1, n_samples // len(videos))
        for video in videos:
            video_df = eval_df[eval_df["video"] == video]
            n = min(n_per_video, len(video_df))
            selected_list.append(video_df.sample(n=n, random_state=seed))
    ...
    for i, row in enumerate(selected.itertuples()):
        ...
        vis = create_visualization(img_path, rho_pred, theta_pred, rho_gt, theta_gt, metrics, row.video)
