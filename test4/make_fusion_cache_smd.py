# -*- coding: utf-8 -*-
"""Build FusionCache for SMD (Experiment A: zero-shot test).

This script is a SMD-adapted version of make_fusion_cache.py.
It expects outputs from: test4/prepare_smd_testset.py

Inputs (relative to project root by default):
  - test4/SMD_GroundTruth.csv        (header: img_name,x1,y1,x2,y2; coords in original pixels)
  - test4/smd_frames/                (exported frames, filenames match img_name)
  - test4/splits/                    (train/val empty, test all)

Outputs:
  - test4/FusionCache_SMD_1024x576/{train,val,test}/<idx>.npy

Notes:
  - Uses MU-SID trained weights (you provide paths below).
  - Labels are computed in the UNet input space (1024x576), consistent with your pipeline.
"""
...
# ----------------------------
# Path setup (so imports work when this file is in ./test4)
# ----------------------------
...
def _read_image_smd(img_dir: str, img_name: str):
    """SMD frames already have extension in img_name."""
...
def calculate_radon_label(x1, y1, x2, y2, img_w, img_h, resize_h, resize_w):
    ...
    Label space follows your existing code:
      - theta in [0, pi) mapped to [0,1] by degrees/180
      - rho mapped to [0,1] over padded sinogram height resize_h
    """
...
def post_process_mask_top_connected(mask_np: np.ndarray) -> np.ndarray:
    """Keep connected sky components touching the top edge (after close)."""
    valid = (mask_np != 255)
...
def build_cache_for_split(df: pd.DataFrame, indices, out_dir: str, seg_model, detector, theta_scan):
    ...
    print(f"[Split] {split_name}: {len(indices)}")
    ...
    for idx in tqdm(indices, ncols=80):
        ...
        bgr = _read_image_smd(IMG_DIR, img_name)
        if bgr is None:
            continue
        ...
        # 5) Compute label in UNet space (scale original coords -> 1024x576)
...
def main():
    ...
    # strict=False because some users train slightly different heads
    ...
    # SMD CSV has header row
    df = pd.read_csv(CSV_PATH)
    expected = {"img_name", "x1", "y1", "x2", "y2"}
    if not expected.issubset(set(df.columns)):
        raise ValueError(f"CSV columns mismatch. Expected at least {expected}, got {set(df.columns)}")
